...
public class ShoppingListService : IShoppingListService
{
    ...
    {
        var lists = await _repository.GetAllByUserIdAsync(userId);
        foreach (var list in lists) SignUrls(list);
        return lists;
    }
    ...
    private ShoppingList SignUrls(ShoppingList list)
    {
        ...
        foreach (var item in list.Items)
        {
            ...
            {
                img.ImageUrl = _fileStorageService.GenerateSignedUrl(img.ImageUrl, TimeSpan.FromHours(1));
                if (!string.IsNullOrEmpty(img.ThumbnailUrl))
                {
                    img.ThumbnailUrl = _fileStorageService.GenerateSignedUrl(img.ThumbnailUrl, TimeSpan.FromHours(1));
                }
            }
        }
        return list;
