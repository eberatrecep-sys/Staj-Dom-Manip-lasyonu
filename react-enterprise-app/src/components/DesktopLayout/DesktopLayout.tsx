...
export const DesktopLayout: React.FC<DesktopLayoutProps> = ({ children }) => {
  ...
  return (
    <div className={`desktop-layout-wrapper ${isDark ? 'dark-theme' : ''}`} style={styles.wrapper}>
      {/* Sidebar */}
      <aside style={{ ...styles.sidebar, backgroundColor: isDark ? '#1a1a1a' : '#ffffff', borderRight: isDark ? '1px solid #333' : '1px solid #eaecf0' }}>
        <div style={styles.profileArea}>
          ...
          />
          <h3 style={{ ...styles.profileName, color: isDark ? '#fff' : '#101828' }}>{userInfo.name}</h3>
          <p style={{ ...styles.profileEmail, color: isDark ? '#a0a0a0' : '#667085' }}>{userInfo.email}</p>
        ...
        <nav style={styles.nav}>
          <button style={{ ...styles.navItem, color: isDark ? '#ddd' : '#344054' }} onClick={() => navigate('/shopping-list')}>
            Listelerim
          </button>
          <button style={{ ...styles.navItem, color: isDark ? '#ddd' : '#344054' }} onClick={() => navigate('/shopping-list?tab=Shared')}>
            Paylaşılanlar
          </button>
          <button style={{ ...styles.navItem, color: isDark ? '#ddd' : '#344054' }} onClick={() => navigate('/shopping-list?tab=Drafts')}>
            Taslaklar
          </button>
          {localStorage.getItem('role') === 'SUPER_ADMIN' && (
            <button style={{ ...styles.navItem, color: isDark ? '#ddd' : '#344054' }} onClick={() => navigate('/admin')}>
        ...
        <div style={styles.footer}>
          <button onClick={toggleTheme} style={{ ...styles.themeToggle, backgroundColor: isDark ? '#333' : '#f9fafb', color: isDark ? '#fff' : '#101828' }}>
            {isDark ? (
      ...
      {/* Main Content Area */}
      <main style={{ ...styles.mainContent, backgroundColor: isDark ? '#121212' : '#f9fafb', color: isDark ? '#fff' : '#101828' }}>
        {children}
      </main>
